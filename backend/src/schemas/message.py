...
class BaseMessage(BaseModel):
    ...
    total_tokens: int = Field(
        default=0, ge=0, description="Total number of tokens (prompt + completion)"
    )

    cost: float = Field(default=0.0, ge=0.0, description="Cost of the message in USD")
...
class MessageCreate(BaseMessage):
    ...
    chat_id: int = Field(
        ..., gt=0, description="ID of the chat this message belongs to"
    )
...
class MessageUpdate(BaseModel):
    ...
    content: Optional[str] = Field(
        None, min_length=1, max_length=10000, description="Updated message content"
    )
